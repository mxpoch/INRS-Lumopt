...
def runSim(params, eps_min, eps_max, x_pos, y_pos, filter_R):
    ...
    fom_N = ModeMatch(monitor_name = 'fom_N', mode_number = 'Fundamental TE mode', direction = 'Forward', target_T_fwd=lambda wl: 0.32*np.kaiser(wl.size, 50), norm_p = 2)
    fom_E = ModeMatch(monitor_name = 'fom_E', mode_number = 'Fundamental TE mode', direction = 'Forward', target_T_fwd=lambda wl: 0.36*np.kaiser(wl.size, 50), norm_p = 2)
    ...
    optimizer = ScipyOptimizers(max_iter=50, method='L-BFGS-B', scaling_factor=1, pgtol=1e-5, ftol=1e-5, scale_initial_gradient_to=0.0)
    
    wavelengths = Wavelengths(start = 289.792*1e-6, stop = 309.793*1e-6, points = 11)
    ...
    opt_E = Optimization(base_script=base_geom, wavelengths = wavelengths, fom=fom_E, geometry=geometry, optimizer=optimizer, use_deps=False, hide_fdtd_cad=True, plot_history=False, store_all_simulations=False)
...
def base_geom(fdtd):
    ...
    fdtd.addrect()
    fdtd.set('name', 'output wg east')
    fdtd.set('x min', opt_size_x / 2)
    fdtd.set('x max', size_x + 40e-6)
    fdtd.set('y', 0)
    fdtd.set('y span', wg_width)
    fdtd.set('z', 0)
    fdtd.set('z span', 220e-7)
    ...
    fdtd.set('y', 0)
    fdtd.set('x', opt_size_x/2 + 20e-6)
    fdtd.set('y span', opt_size_y/2)
    fdtd.setglobalmonitor("frequency points", 100)

    fdtd.addmesh()
    fdtd.set('name', 'fom_mesh_E')
    fdtd.set('override x mesh', True)
    fdtd.set('dx', dx)
    fdtd.set('override y mesh', False)
    fdtd.set('override z mesh', False)
...
if __name__ == '__main__':
    ...
    size_y = 140		#< Since we use symmetry, this is only have the extent along the y-axis (in nm
    delta_y = 2	#< Size of a pixel along y-axis (in nm)
	
    filter_R = 18	#< Radius of the smoothing filter which removes small features and sharp corners (in nm)
    ...
    #x_half = int((x_points - 1)/2)
    # initial condition based to the north port
    #initial_cond = np.vstack((0.5*np.ones((x_half+1+20, y_points)), np.zeros((x_half-20, y_points))))
    #initial_cond=initial_cond + np.triu(np.full((initial_cond.shape[0], initial_cond.shape[0]), 1))[:,:71]   
